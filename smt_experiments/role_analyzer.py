# Taken from https://github.com/gravitational/rbac-linter/blob/2e64c6e437f2309cbd16b4859bb39d6b73807360/role_analyzer.py
# TODO: do we need to put credits in here or something?
...
def is_role_template(role) -> bool:
    ...
    if 'allow' in spec:
        allow = spec['allow']
        groups = [allow[constraint_type].values() for constraint_type in entity_types.keys() if
                  constraint_type in allow]
...
def traits_as_z3_map(
    ...
    if concrete_traits is not None and any(concrete_traits):
        ...
        excluded_values_excluded = z3.Implies(z3.And(is_included_key, is_excluded_value),
