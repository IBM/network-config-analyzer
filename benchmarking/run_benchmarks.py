...
from benchmarking.utils import get_benchmark_result_file, BenchmarkProcedure, \
    get_experiment_results_dir, Benchmark
...
def benchmark_filter(benchmark: Benchmark) -> bool:
...
def run_benchmarks(experiment_name: str, example_benchmark_only: bool = False, tests_only: bool = False,
    ...
    for i, benchmark in enumerate(benchmarks, 1):
        for benchmark_procedure, func in benchmark_procedure_to_func.items():
            logger.info(f'{i} / {len(benchmarks)} - running {benchmark_procedure.name} on {benchmark.name}.')
    ...
    create_report(experiment_name, benchmarks)
