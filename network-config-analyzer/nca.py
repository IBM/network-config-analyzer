...
def _execute_pair_configs_query(query_name, network_config1, network_config2, output_config, expected_output=None):
    """
    Runs a query between two network configs
    :param str query_name: the name of the arg.query
    :param NetworkConfig network_config1: first network config
    :param NetworkConfig network_config2: second network config
    :param OutputConfiguration output_config: dict object
    :param str expected_output: a file path to the expected output
...
def run_args(args):
    ...
    resources_handler = ResourcesHandler(base_np_list, base_resource_list, base_ns_list, base_pod_list,
                                         np_list, resource_list, ns_list, pod_list)

    network_config = resources_handler.get_network_config()
    if pair_query_flag:
        base_network_config = resources_handler.get_base_network_config()
        return _execute_pair_configs_query(query_name, base_network_config, network_config,
...
def nca_main(argv=None):
    ...
    manual_or_automatic.add_argument('--interferes', type=str, nargs='?', const='',
                                     help='Network policies (policy name/file/dir/GHE url/cluster-type) '
                                          'for interference checking')
    manual_or_automatic.add_argument('--forbids', type=str, nargs='?', const='',
                                     help='Network policies (policy name/file/dir/GHE url/cluster-type) '
                                          'specifying forbidden connections')
    manual_or_automatic.add_argument('--permits', type=str, nargs='?', const='',
                                     help='Network policies (policy name/file/dir/GHE url/cluster-type) '
    ...
    parser.add_argument('--base_np_list', '-b', type=_network_policies_valid_path,
    ...
    parser.add_argument('--base_resource_list', '-rb', type=_resource_list_valid_path,  # action='append',
                        help='Filesystem or GHE location of base network resources ')
