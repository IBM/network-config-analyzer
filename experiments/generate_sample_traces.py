...
from experiments.canonical_hyper_cube_set_tracker.canonical_hyper_cube_set_tracker import CanonicalHyperCubeSetTracker as CanonicalHyperCubeSet
import experiments.canonical_hyper_cube_set_tracker.canonical_hyper_cube_set_tracker as tracker
...
class TestCanonicalHyperCubeSetMethods(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        filename = 'example_traces/' + self.id().split('.')[-1] + '.json'
        file = Path(filename)
        with file.open('w') as f:
            json.dump(tracker.RECORDS, f, indent=4)
