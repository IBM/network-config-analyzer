...
class CanonicalHyperCubeSetOriginal:
    """
    ...
    * An empty object has at least one active dimension and an empty 'layers' map.
    ...
    def __eq__(self, other):
        if not isinstance(other, CanonicalHyperCubeSetOriginal):
            return False
    ...
    def create_from_cube(all_dims, cube, cube_dims):
        ...
        cube, cube_dims, is_empty = CanonicalHyperCubeSetOriginal._reduce_cube(cube, cube_dims)
        if is_empty:
            return CanonicalHyperCubeSetOriginal(all_dims)
        if not cube:
            # if the reduced cube is empty -> it was the entire space
            return CanonicalHyperCubeSetOriginal(all_dims, True)
        return CanonicalHyperCubeSetOriginal._create_from_cube_aux(all_dims, cube, cube_dims)
    ...
    def _create_from_cube_aux(all_dims, cube, cube_dims):
        ...
        :return: CanonicalHyperCubeSet object, containing the input cube
        """
        res = CanonicalHyperCubeSetOriginal(all_dims)
        res.active_dimensions = cube_dims
        if len(cube) == 1:
            res.layers[cube[0]] = CanonicalHyperCubeSetOriginal.empty_interval
            return res
        res.layers[cube[0]] = CanonicalHyperCubeSetOriginal._create_from_cube_aux(all_dims, cube[1:], cube_dims[1:])
        return res
    ...
    def copy(self):
        res = CanonicalHyperCubeSetOriginal(self.all_dimensions_list)
    ...
    def _and_aux(self, other):
        ...
        :type other: CanonicalHyperCubeSetOriginal
        :return: self
        """
    ...
    def or_aux(self, other):
        ...
        for self_layer in self.layers:
            ...
            for other_layer in other.layers:
                ...
                if self._is_last_dimension():
                    res_layers[common_elem] = CanonicalHyperCubeSetOriginal.empty_interval
                    continue
    ...
    def add_cube(self, cube_to_add, cube_dimensions=None):
        ...
        for dim_value in cube_to_add:
            if not dim_value:
                return
        cube_obj = CanonicalHyperCubeSetOriginal.create_from_cube(self.all_dimensions_list, cube_to_add, cube_dimensions)
        res = self | cube_obj
    ...
    def add_hole(self, hole_to_add, hole_dimensions=None):
        ...
        for dim_value in hole_to_add:
            if not dim_value:
                return
        cube_obj = CanonicalHyperCubeSetOriginal.create_from_cube(self.all_dimensions_list, hole_to_add, hole_dimensions)
        res = self - cube_obj
        self._override_by_other(res)
    ...
    def build_new_active_dimensions(self, new_active_dimensions):
        ...
        if self.active_dimensions[0] == new_active_dimensions[0]:
            if not self._is_last_dimension():
                ...
                self.active_dimensions = new_active_dimensions
                new_sub_elem = CanonicalHyperCubeSetOriginal(self.all_dimensions_list)
                ...
                new_sub_elem.layers[dim_all_values] = CanonicalHyperCubeSetOriginal.empty_interval
    ...
    def _check_active_dimensions(self):
        ...
        act_dim = None
        for sub_elem in self.layers.values():
            if isinstance(sub_elem, CanonicalHyperCubeSetOriginal):
    ...
    def _remove_some_active_dimensions(self, new_active_dimensions):
        ...
        if self.active_dimensions[0] == new_active_dimensions[0]:
            ...
            # case 1 (c) : not last dimension + no more new_active_dimensions -> should become last dimension
                ...
                for layer_elem in self.layers:
                    self.layers[layer_elem] = CanonicalHyperCubeSetOriginal.empty_interval
                self.active_dimensions = new_active_dimensions
                return
