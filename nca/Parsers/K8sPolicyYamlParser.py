...
class K8sPolicyYamlParser(GenericYamlParser):
    ...
    def parse_ingress_egress_rule(self, rule, peer_array_key, rule_index, policy_kind, policy_name):
        """
        ...
        :rtype: K8sPolicyRule
        """
        is_ingress = peer_array_key == 'from'
    ...
    def parse_ingress_rule(self, rule, policy_selected_pods, rule_index, policy_kind, policy_name):
        """
        ...
        :rtype: K8sPolicyRule
        """
        res = self.parse_ingress_egress_rule(rule, 'from', rule_index, policy_kind, policy_name)
        self.verify_named_ports(rule, policy_selected_pods, res.port_set)
        return res

    def parse_egress_rule(self, rule, rule_index, policy_kind, policy_name):
        """
        ...
        :rtype: K8sPolicyRule
        """
        res = self.parse_ingress_egress_rule(rule, 'to', rule_index, policy_kind, policy_name)
    ...
    def parse_policy(self):
        ...
        if ingress_rules:
            for rule_index, ingress_rule in enumerate(ingress_rules):
                res_policy.add_ingress_rule(self.parse_ingress_rule(ingress_rule, res_policy.selected_peers, rule_index, res_policy.policy_kind, res_policy.full_name()  ))

        egress_rules = policy_spec.get('egress', [])
        if egress_rules:
            for rule_index, egress_rule in enumerate(egress_rules):
                res_policy.add_egress_rule(self.parse_egress_rule(egress_rule, rule_index, res_policy.policy_kind, res_policy.full_name()), )
